...
{
    // uint256 public constant COST = 1 ether; // REMOVED
    
    Burned public unrealB;
    ...
    function setUnrealB(address _unrealB) external onlyOwner {
        unrealB = Burned(_unrealB);
    }
    ...
    // --- Transfer From with Burn and Mint Logic ---
    ...
    function transferFrom(
        ...
        // Custom Logic: Burn from 'from', Mint 'UnrealB' to 'to'
        // This effectively pays 'to' with a receipt token by burning the payment token.
        _burn(from, amount);
        
        if (address(unrealB) != address(0)) {
            unrealB.mint(to, amount);
        }
